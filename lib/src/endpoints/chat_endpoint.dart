...
class ChatEndpoint extends Endpoint with AuthProtected {
  ...
  Future<String> requestChat(Session session) async {
    ...
    // ← جيب الأدمن اللي بيكلمه دلوقتي لو موجود
    final activeConv = await ChatConversation.db.findFirstRow(
      session,
      where: (c) =>
          c.customerId.equals(customerId) &
    ...
    final excludedAdminId = activeConv?.adminId;

    // ← بعت الـ notification مع الـ excludedAdminId في الـ message
    session.messages.postMessage(
      'admin_notifications',
      ChatEvent(
        type: 'new_request',
        ...
        senderId: customerId,
        senderRole: 'customer',
        message: excludedAdminId?.toString() ?? '',
        sentAt: DateTime.now().toIso8601String(),
      ),
    );
  ...
  Future<void> sendMessage(
    ...
    if (conversation.status != 'active' && conversation.status != 'pending') {
      throw Exception('conversation_closed');
    }
  ...
  Future<String> getMyActiveChats(Session session) async {
    ...
    final conversations = await ChatConversation.db.find(
      session,
      where: (c) =>
          c.adminId.equals(adminId) & c.status.equals('active'),
      orderBy: (c) => c.updatedAt,
    );
